...
function undo() {
  ...
  redoStack.push(ctx.getImageData(0, 0, canvasRef.value.width, canvasRef.value.height))
  if (history.length > 0) {
      ...
      <div class="tools main-bg-col justify-content-between align-items-center">
        <div class="nav justify-content-center text-white logofont">
          ...
          <i
            ...
            @click="setColor(strokeColor === '#ffffff' ? lastValidColor : strokeColor)"
          ></i>
          <i class="bi bi-eraser-fill" @click="useEraser"></i>
          <i class="bi bi-trash3-fill" @click="clearCanvas"></i>
          <i class="bi bi-arrow-counterclockwise" style="color: white" @click="undo"></i>
          <i class="bi bi-arrow-clockwise" style="color: white" @click="redo"></i>
          <input type="range" min="1" max="20" v-model="strokeWidth" @input="onSizeInput" /><i
